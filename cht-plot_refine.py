...
def export_TEM(file_name):
    ...
    bi_data = pd.read_csv('Bi_positions_refined.txt', sep='\s+', header=None)
    fe_data = pd.read_csv('Fe_positions_refined.txt', sep='\s+', header=None)
    ...
    with open('result_refined.txt', "w") as out_file:
        x_list = []
        y_list = []
        u_list = []
        v_list = []
        ...
        # print('------------------------')
        plt.savefig('result_refined.png',dpi=300)
